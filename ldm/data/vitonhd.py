...
from ldm.data.posemap import get_coco_body25_mapping
from ldm.data.posemap import kpoint_to_heatmap
...
class VitonHDDataset(data.Dataset):
    def __init__(self,
                 ...
                 outputlist: Tuple[str] = ('c_name', 'im_name', 'cloth', 'image', 'im_cloth', 'shape', 'pose_map',
                                           ...
                                           'captions', 'category', 'warped_cloth', 'clip_cloth_features', 'seg_mask', 'dense_pose_map', 'cloth_mask','parse_agnostic','open_pose'),
        ...
        possible_outputs = ['c_name', 'im_name', 'cloth', 'image', 'im_cloth', 'shape', 'im_head', 'im_pose',
                            ...
                            'category', 'warped_cloth', 'clip_cloth_features' , 'seg_mask','dense_pose_map', 'cloth_mask', 'parse_agnostic','open_pose']
    ...
    def __getitem__(self, index):
        ...
        if "cloth_mask" in self.outputlist:
            cloth_mask = Image.open(os.path.join(dataroot, self.phase, 'cloth-mask', c_name))
        ...
        if 'open_pose' in self.outputlist: 
            # load pose points
            pose_name = im_name.replace('.jpg', '_rendered.png')
            pose_map = Image.open(os.path.join(dataroot, self.phase, 'openpose_img', pose_name))
            pose_map = transforms.Resize(self.width, interpolation=2)(pose_map)
            open_pose = self.transform(pose_map)  # [-1,1]
        ...
        if 'parse_agnostic' in  self.outputlist:
            parse_name =  im_name.replace('.jpg', '.png')
            image_parse_agnostic = Image.open(os.path.join(dataroot, self.phase, 'image-parse-agnostic-v3.2', parse_name))
            image_parse_agnostic = transforms.Resize(self.width, interpolation=0)(image_parse_agnostic)
            ...
            new_parse_agnostic_map = torch.FloatTensor(13, self.height, self.width).zero_()
        ...
        if "seg_mask" in self.outputlist:
            parse_name = im_name.replace('.jpg', '.png')
            seg_mask = Image.open(os.path.join(dataroot, self.phase, 'image-parse-v3', parse_name))
            # seg_mask = seg_mask.resize((self.width, self.height))
            seg_mask = transforms.Resize(self.width, interpolation=0)(seg_mask)
            # seg_mask = torch.from_numpy(np.array(seg_mask)[None]).long()
            seg_mask = self.transform(seg_mask.convert('RGB'))

            # seg_mask = seg_mask.resize((self.width, self.height), Image.NEAREST)
            # seg_mask = transforms.ToTensor()(seg_mask) * 255
        
        if "dense_pose_map" in self.outputlist:
            dense_pose_map = im_name.replace('image', 'image-densepose')
            dense_pose_map = Image.open(osp.join(dataroot, self.phase, 'image-densepose', dense_pose_map))
            dense_pose_map = transforms.Resize(self.width, interpolation=2)(dense_pose_map)
            dense_pose_map = self.transform(dense_pose_map)  # [-1,1]
...
if __name__ == "__main__":
    ...
    dataset = VitonHDDataset(
            dataroot_path='/home/ock/aigc/vition-HD',
            phase='train',
            order='paired',
            radius=5,
