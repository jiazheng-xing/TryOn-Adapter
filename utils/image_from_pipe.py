...
from .AutoencoderKL import AutoencoderKL2
from .emasc import EMASC
from .inversion_adapter import InversionAdapter
...
def extract_save_vae_images(vae: AutoencoderKL2, emasc: EMASC, test_dataloader: torch.utils.data.DataLoader,
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        if "category" in batch.keys():
            ...
            category = ["upper"] * len(batch["GT"])

        if emasc_type != "none":
            # Extract intermediate features from 'im_mask' and encode image
            posterior_im, _ = vae.encode(batch["GT"])
            ...
            processed_intermediate_features = mask_features(processed_intermediate_features,(1- batch["inpaint_mask"]))
            ...
            posterior_im = vae.encode(batch["GT"])
            latents = posterior_im.latent_dist.sample()
            generated_images = vae.decode(latents).sample

        # Save images
        for gen_image, cat, name in zip(generated_images, category, batch["file_name"]):
            gen_image = (gen_image + 1) / 2  # [-1, 1] -> [0, 1]
            if not os.path.exists(os.path.join(save_path, cat)):
                os.makedirs(os.path.join(save_path, cat))
            torchvision.utils.save_image(gen_image, os.path.join(save_path, cat, name), quality=100)
