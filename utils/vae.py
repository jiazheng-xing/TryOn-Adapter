...
class Decoder(nn.Module):
    def __init__(
        ...
        self.use_blend_fusion = False
        # if self.use_blend_fusion: 
        #     self.blend_fusion = nn.ModuleList()
        #     for blend_in_ch, blend_out_ch in zip(feature_channels, feature_channels):
        #         self.blend_fusion.append(nn.Conv2d(blend_in_ch, blend_out_ch, kernel_size=3, bias=True, padding=1, stride=1))
        #     self.blend_fusion.apply(self._init_weights)
    ...
    def forward(self, z, intermediate_features=None, int_layers=None):
        ...
        if intermediate_features:
            ...
            # up
            for up_block, int_feat, blend_block in zip(self.up_blocks, intermediate_features, self.blend_fusion):

                sample += int_feat
                if self.use_blend_fusion:
                    sample = sample + blend_block(sample)
        ...
        if int_layers and self.use_blend_fusion and 1 in int_layers:
            sample = sample + self.blend_fusion[-1](sample)
        sample = self.conv_out(sample)
